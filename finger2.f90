...
! Fingerprintak d-ko dimentsioa edukiko du atomo mota
! bakoitzerako. Gainera, fingerprintaren 0 posizioan
! atomo horren masa molekularra egongo da.
...
allocate(fingerprint(n_inp,sum(numIons),d,3),a(d),atom_list(sum(numIons),2))
...
do i = 1, sum(numIons)
    ...
    write(unit=124, fmt='(4f20.10)') 0.0, real(atom_list(i,2)), real(atom_list(i,2)), real(atom_list(i,2))
    do j = 1, n_inp
        do k = 1, d
            write(unit=124, fmt='(4f20.10)') a(k), fingerprint(j,i,k,:)
        enddo
    enddo
